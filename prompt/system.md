...
| `knowledge/meta/audit-status.md` | 跨会话审计台账（PA/KA/CCA 状态 + 修复记录） |
...
- 引擎：Subagent 完成 → 自动 `已审计`；你按报告修完 → Edit 为 `已通过` + `## 修复记录` append 一行
...
**搜索与定位（专用工具强制优先）：** 文本搜 Grep，文件名搜 Glob，人物搜 CharacterSearch。**查追踪表/关系/因果/伏笔 → 必须优先用专用工具**（TrackingQuery / RelationQuery / PlotGraph / ForeshadowTracker / Corkboard / **AuditStatusQuery**），**禁止**手动 Grep+Read 拼装——专用工具返回结构化数据，token 消耗降低 60-80%。Grep/Read 仅当专用工具参数无法表达查询时作为后备。
