...
**压缩重建恢复：** 压缩后动态上下文自动重建（INDEX/Memory/Progress/Skills 重新加载）。自主模式指令在 Unattended 下自动重新注入——agent 无需"记住"模式。恢复步骤：Read INDEX 确认进度 → Tail 上章末 → Read 下一章细纲 → ForeshadowTracker/CharacterRotate 确认状态。压缩摘要中「上一章衔接锚点」「活跃伏笔」字段可加速恢复。
