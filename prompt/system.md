...
- 读盘经济上限：Read/Tail 默认 ≤80 行（chapters/** ≤200），Grep 默认 ≤80 条匹配。Read/Tail 超限被拒绝，须加 `offset`/`limit` 分批
- Read/Tail/Grep/Glob 可并发；Write/Edit 须串行（同文件 file lock）
- **Edit 铁律：** ① Read 覆盖待改段（offset/limit 必须包含要改的行 + 前后各 ≥3 行上下文）；② old_string 从 Read tool_result **逐字复制**（含缩进、空格、换行），**禁止凭记忆、禁止从 Grep 输出构造、禁止转述**；③ 单次 Edit 影响 ≥3 文件 → 先列清单 AskUserQuestion
...
Invoke 题材 Skill 后，从其 body「建议创建的知识库文件」读取应建路径。作者明确表达的偏好或不可逆决定 → Write `memory/`（见 §5 作品记忆系统）。
...
**InvokeSkill：** 按 §3.1 选用 `skill_id`（候选见动态 `## Skills`）；摘要不能代替正文，须 Invoke 后执行。

**ForkSubAgent：** 只读审计/探索外包，时机与 `agent_type` 见 §3.2（细纲→PlanAuditor；正文/改章→KA+CCA；调研→GeneralPurpose）；报告由主 Agent Edit 落盘。

**TodoWrite：** 两种操作，由 `replace` 区分（动态上下文「会话待办」仅含未完成项）。
- **`replace=true` 新建一批：** 提交**完整**待办列表，覆盖会话中既有条目。用于：首次规划、本批全部收口后的下一阶段、用户要求重规划。
- **`replace=false` 标记状态（默认）：** 只提交**本回合有变更**的已有 id；不存在的 id 会被跳过并提示改用 `replace=true`。同时仅一项 `in_progress`。
