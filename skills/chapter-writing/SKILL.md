...
skill_kind: workflow
allowed-tools: Read, Write, Edit, Glob, Grep, Bash, CharacterSearch, PlotGraph, ChapterRead, InvokeSkill, TodoWrite, Stats, ForkSubAgent
---
...
4. **查人物**：CharacterSearch 查询细纲中所有出场人物。对每人 Read 人物卡，取各演变日志**最后行**：
   - 性格演变日志 → 当前性格状态
   - 关系演变日志 → 与他人当前关系
   - 已知信息演变日志 → 角色 POV 信息边界（该角色知道什么）
   - 修为/功法演变日志（如有）→ 当前战力
...
7. **读上一章**（第 2 章起必做；第 1 章跳过）：
   ...
   **先判断是否需要 Read**：如果上一章的完整正文已在当前对话上下文中（如上一轮刚 Write/Edit/ChapterRead 过该章），且未被后续 Edit 覆盖，则直接分析上下文中的内容即可，**跳过 ChapterRead**，节省上下文。

   如需读取，使用：
   ```
   ChapterRead(chapter="chapters/chapter-{N-1:03d}.md", context="tail", lines=100)
   ```
...
| 句式 | 不用「首先…其次…」排比；长短句交替，避免句长均匀 |
| 风格 | 不写说明书式平滑叙述；允许 1–2 处与主线无关的闲笔 |
| 情感 | 禁止空贴标签；用动作、对话、生理反应、具体记忆呈现 |
| 节奏 | 重要场景细写、过渡略写 |
| 创新 | 按细纲推进但拒绝最安全、最套路的接龙写法 |
| 连贯 | 第 2 章起写前已读上章 tail；开篇接上钩子，勿重复上章末段句式 |
...
4. **同一次 assistant 消息**内并行 Fork **5 项** Subagent（task 均含 `chapters/chapter-NNN.md`；EmotionAnalyzer 含 POV/主角名）：
   - LogIntegrityChecker · ConsistencyChecker · DialogueAnalyzer · PacingAnalyzer · EmotionAnalyzer
