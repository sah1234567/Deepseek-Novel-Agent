...
- **字数**：`Stats(chapter="N")` 统计本章字数，确认 2000–4000。
  **★ 字数调整一律用 Edit 局部修改，禁止 Write 全文重写——重写引入新错误的代价远超局部修。**
  ...
  - **超过 4000 字**：逐章 Grep 定位冗余（`search_root=chapters`、`glob=chapter-NNN.md`、`head_limit=0`）— `然后`、`——|--|—|–| - `（破折号须用完整 pattern，禁止只写 `--`）、`首先\|其次\|一方面\|另一方面`、Markdown 标记 → 针对命中段逐段 Edit。精修后 Stats 复验；仍超标 → 征求作者意见是否拆分章节。
