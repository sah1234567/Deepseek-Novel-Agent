...
use crate::message_bridge::{
    assistant_from_completion, to_llm_messages_traced, RepairTraceContext,
};
use crate::session_llm::{build_chat_client, SessionLlmSnapshot};
use crate::subagent::{build_fork_child, SubagentJob, SubagentJobKind};
use crate::subagent_helpers::{
    execute_subagent_tool_batch, fork_child_push, subagent_push_tool_results,
};
use crate::subagent_llm_turn::{
...
async fn subagent_apply_completion_tools(
    ...
    fork_dispatch: Option<
        std::sync::Arc<std::sync::Mutex<crate::streaming_tool_dispatch::StreamingToolDispatch>>,
    >,
    event_tx: Option<&mpsc::UnboundedSender<Event>>,
) -> Result<SubagentTurnOutcome, AgentError> {
    ...
    let fork_ctx = crate::subagent_helpers::subagent_fork_tool_context(shared);
