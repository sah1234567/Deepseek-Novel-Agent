//! Helpers for [`crate::subagent_runner::run_subagent_job`] (tool context, tool batch, stream events).

use crate::agent::merge_tool_always_allow;
use crate::message_bridge::{parse_tool_call_input, tool_result_message};
...
pub(crate) fn subagent_push_tool_results(
    ...
    fork_child_push(
        ...
        fork_run_id,
        child_messages,
        crate::message_bridge::assistant_from_completion(completion),
    )?;
...
mod tests {
    ...
    fn subagent_push_tool_results_persists_messages() {
        ...
        let turn_ctx = TurnContext::new(1, 8);
