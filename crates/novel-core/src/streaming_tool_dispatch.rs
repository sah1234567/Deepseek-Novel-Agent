use crate::message_bridge::parse_tool_call_input;
...
impl StreamingToolDispatch {
    ...
    pub(crate) fn executor_mut(&mut self) -> &mut StreamingToolExecutor {
        self.executor
            .as_mut()
...
mod tests {
    ...
    async fn poll_ui_results_emits_once_per_tool() {
        ...
        for _ in 0..100 {
            if !dispatch.executor_mut().peek_completed_results().is_empty() {
                break;
            }
