...
use crate::engine::session_llm::{apply_session_usage, build_chat_client, SessionLlmSnapshot};
use crate::message::assistant_from_completion;
use crate::message::yield_missing_tool_result_blocks;
...
impl TurnFinalizeSink for ForkTranscriptSink<'_> {
    ...
    fn push_tool_stub(&mut self, stub: ChatMessage) -> Result<(), AgentError> {
        crate::subagent::fork_transcript::persist_fork_message(self.db, self.fork_run_id, &stub)?;
...
pub struct FinalizeStreamCancelParams<'a> {
    ...
    /// Main Agent: true (update StatusBar snapshot). Fork subagent: false.
    pub update_context_snapshot: bool,
}
...
pub async fn finalize_stream_cancel(
    ...
    if let Some(ref u) = usage {
        apply_session_usage(
            ...
            &params.llm_snap,
            params.event_tx,
            params.update_context_snapshot,
        );
    } else {
        let _ = params
            .shared
...
mod tests {
    ...
    async fn finalize_stream_cancel_persists_partial_and_stubs() {
        use crate::interrupt::finalize::{FinalizeStreamCancelParams, MainSessionSink};
        ...
        let llm_snap = crate::engine::session_llm::read_session_llm(&shared);
