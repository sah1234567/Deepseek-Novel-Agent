//! Subagent LLM stream fetch (extracted from `run_subagent_job`).
...
use crate::session_llm::{apply_session_usage, SessionLlmSnapshot};
use crate::streaming_tool_dispatch::StreamingToolDispatch;
use crate::subagent_helpers::fork_child_push;
use crate::subagent_helpers::{forward_subagent_stream_event, subagent_fork_tool_context};
...
pub(crate) fn subagent_after_completion(
    ...
    if let Some(u) = &completion.usage {
        apply_session_usage(shared, u, llm_snap, event_tx);
    }
...
mod tests {
    use super::*;
    use crate::session_llm::{build_chat_client, read_session_llm};
    ...
    async fn fetch_subagent_live_stream_via_wiremock() {
        ...
        let fetch = fetch_subagent_llm_completion(
            ...
            "audit",
            &fork_run_id,
            &crate::message_bridge::to_llm_messages(&child_messages),
            &[],
            &snap,
            None,
            &mut child_messages,
