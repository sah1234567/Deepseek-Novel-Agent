...
| Tauri 手动 fork | 调试用，fire-and-forget，不注入主会话 |
...
**AppStatus：** 包含当前 session ID、权限模式、作品名、turn 序号、待办列表、token 统计（三分类累计 + 会话总量）、项目初始化状态、hook 与中断状态等，由前端 `useAppStatus` 轮询。
...
**前端 invoke 参数：** camelCase（例：`resume_session({ sessionId })`、`get_session_messages({ sessionId: null })`、`update_session_todo({ todoId, status })`）。

**`session-resumed` 事件：** `create_session` / `resume_session` / 切换作品成功后触发；`useAgent` 清 streaming 状态并 `hydrateMessages`。

### 2.6 用户中断

`AbortController` 贯穿 LLM 流与工具：Esc → 立即断开 SSE 流；发送新消息 → 中断当前流但不添加中断消息。

中断后：partial assistant 持久化 + 缺失 tool_result 补填。为保持 session token 计数准确，中断后独立发送一条轻量请求（`max_tokens=1`）利用 prefix cache 获取 prompt_tokens 估算值——注意此请求的三分类拆分不等于原始请求，仅为估算。
