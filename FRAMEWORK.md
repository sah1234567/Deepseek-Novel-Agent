...
**引擎队列（`EngineShared`）：** `subagent_queue`（`SubagentWorkQueue` / `PendingSubagentWork`）统一承接 ForkSubAgent 与 PostToolUse hook；`drain_subagent_jobs` 并行 spawn `run_subagent_job` 后 join。`session_llm`（`SessionLlmSnapshot`）保存主 Agent 最近一次 API 的 model/thinking，供 drain 与子 Agent 复用（替代已移除的 `active_llm`）。`drain_in_progress` 在 drain 期间为 true；`AppStatus.hook_running` 与之同义（保留字段名供 Tauri 前端）。
...
**AppStatus：** 包含当前 session ID、权限模式、作品名、turn 序号、待办列表、token 统计（三分类累计 + 当前上下文）、项目初始化状态、`hook_running`（= `drain_in_progress`，子 Agent drain 中）与中断状态等。`useAppStatus` 每 5s 轮询 `get_app_status`；`turn-complete` / `session-resumed` 时即时 refresh。
