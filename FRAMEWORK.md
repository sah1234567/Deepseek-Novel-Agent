...
React Frontend (ui/src/)
  ChatPanel · StatusBar（含 Todo 下拉）· SettingsPanel（弹窗）· FileTreePanel
...
| **Session 重建压缩** | 超阈值时：**先** archive 全量 → `refresh_system_dynamic_sections`（AGENTS/Workspace 冻结；Index/Memory/Progress/**Skills 摘要** 读盘刷新）→ `[上下文刷新]` user（Skill 全文 + 摘要）→ 5 轮 ReAct。`compaction-progress` → 前端 **CompactionBanner**（已接入）。连续 3 次失败静默 skip（重试 UI 为后续 issue） |
| **Session 双轨存储** | `message_archive`（UI 全历史，按 `compaction_epoch`）+ `messages`（API 工作集）；前端 `get_session_transcript` hydrate |
...
send_message
  ...
  → LLM 流式调用 + 流中 Tool 调度
  → drain pending hooks（PostToolUse → KnowledgeAuditor subagent）
  → drain pending forks（ForkSubAgent → 并行执行 → join → 注入摘要报告）
...
| `context_tokens` / StatusBar「当前上下文」 | **最近一次** API 的 `hit+miss+comp`（覆盖写入；主 Agent 与 SubAgent 调用均会更新 DB 字段） | `accumulate_session_tokens` |
...
| 路径 | 说明 |
|------|------|
| `ForkSubAgent` 工具 | 主会话 LLM 调用；入队后同步等待本批全部完成，注入一条摘要报告 |
| PostToolUse 自动触发 | settings 启用 matcher 后，自动运行 KnowledgeAuditor subagent（轻量扫描，**不注入**主会话） |
...
Agent prompt 文件位于 `prompt/agents/*.md`，编译期嵌入。可 fork 类型：KnowledgeAuditor、ChapterCraftAnalyzer、GeneralPurpose。
...
| 事件 | 用途 |
|------|------|
| `sub-agent-started` / `sub-agent-complete` | `SubAgentForkCard`；payload 含 `forkRunId`、`source`（`tool` \| `hook`） |
...
**前端：** ForkSubAgent 工具卡与 Hook 独立卡（`SubAgentForkCard`）右侧 **进入** → `SubAgentOverlay`（`TranscriptView mode=fork`）；**查看返回内容** 在卡内展开摘要。主列表经 Transcript FSM 分段渲染；无独立 SubAgentReportCard / TodoPanel。
...
**Compaction 后（API 工作集）：** system（AGENTS/Workspace 冻结 + Index/Memory/Progress/Skills 摘要 读盘刷新）→ `[上下文刷新]` user（Skill 全文 + 会话摘要）→ 最近 5 轮 ReAct。Memory / INDEX / Progress **仅在 system 对应节**，不在 `[上下文刷新]`。

**UI 全历史：** 每次压缩前整表写入 `message_archive`；前端 archive 区 + `CompactionDivider` + active 工作集。

### 2.5 前端状态与 IPC

**AppStatus：** 包含当前 session ID、权限模式、作品名、turn 序号、待办列表、token 统计（三分类累计 + 当前上下文）、项目初始化状态、hook 与中断状态等。`useAppStatus` 每 5s 轮询 `get_app_status`；`turn-complete` / `session-resumed` 时即时 refresh。

| 领域 | 主要 Command |
|------|----------------|
| 作品 | `list_works`, `create_work`, `open_work` |
| 会话 | `create_session`, `resume_session`, `list_sessions`, `get_session_transcript` |
...
**前端 invoke 参数：** camelCase（例：`resume_session({ sessionId })`、`get_session_transcript({ sessionId: null })`、`update_session_todo({ todoId, status })`）。

**`session-resumed` 事件：** `create_session` / `resume_session` / 切换作品成功后触发；`useAgent` 清 streaming 状态并 `hydrateMessages`（`get_session_transcript` → archive + active）。
...
```powershell
# Windows 全量 CI（与 GitHub rust-windows / ci-windows-gate.sh 一致）
.\scripts\ci-windows.ps1

# 跨平台本地 CI（Windows 同上；Linux/macOS → ci-pr-gate）
