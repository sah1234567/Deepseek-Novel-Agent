...
| **检查类 Subagent** | ConsistencyChecker / LogIntegrityChecker / 三 Analyzer 只读报告；**写章/改稿后 system prompt 要求同批 Fork 5 项**（引擎不硬编码，由 prompt 约束）；GeneralPurpose 13 工具白名单；LogIntegrityChecker 另可 PostToolUse Hook opt-in |
...
| **读盘经济** | `prompt/system.md` §2.3：Grep/搜索定位 → Read offset/limit 或 ChapterRead head/tail/range → 确需时才 full Read；工具 description 与 Checker/Analyzer prompt 一致 |
...
invoke('send_message')
  ...
  → handle_message_with_events
    ...
    → drain_pending_hooks（PostToolUse 自动触发 LogIntegrityChecker subagent，source=`hook`）
...
**ForkSubAgent 工具路径**完成后 `inject_sub_agent_report` 向主会话注入 **一条** `[子 Agent 完成: {type}]` 摘要（含 `fork_run_id` UI 元数据）；完整 transcript 写入 `fork_messages`，**不**进入主 LLM prompt。PostToolUse 路径（LogIntegrityChecker subagent，`source=hook`）**故意不 inject**，避免污染主会话上下文；UI 经 StatusBar / SubAgentOverlay 查看。
...
| PostToolUse 自动触发 | settings 启用 matcher 后，PostToolUse 管道入队 **LogIntegrityChecker subagent**（`source=hook`）；`run_log_integrity_checker` 同步执行；**不 inject** 主会话 |
...
Agent prompt 文件：`prompt/agents/*.md`（`include_str!` 编译期嵌入）。类型解析统一经 `AgentType::parse`；可 fork 列表见 `FORKABLE_AGENT_TYPE_NAMES`（含 LogIntegrityChecker、GeneralPurpose）。

**GeneralPurpose 权限：** 精选 13 工具白名单（Read/Write/Edit/Glob/Grep/CharacterSearch/PlotGraph/ChapterRead/Stats/InvokeSkill/ImpactAnalysis/TodoWrite/ConsistencyCheck）；无 ForkSubAgent 工具（禁止嵌套 fork），无 Bash。

**代码审查清单（禁止替 Agent 决策）：**

- 无 `allow_chapter_write` / `validate_chapter_write` / 按路径禁止 Write
- 无 `main_tool_names` 裁切 ConsistencyCheck
- 无 Write 后默认自动 LogIntegrityChecker 入队（除非用户 opt-in hooks）
...
| 段 | 来源 |
|----|------|
| 静态层 | `prompt/system.md`（含读盘经济 §2.3、写后 5 项 Fork 规范） |
| AGENTS.md | 作品根 |
