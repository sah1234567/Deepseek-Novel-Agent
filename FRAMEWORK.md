...
| **自主写作模式** | Unattended 权限下，`prompt/autonomous-writing.md` 自动注入 system prompt（编译期嵌入，引擎根据权限模式决定）。含自主循环规则、审计降频策略、暂停条件。压缩重建后根据 `permission_mode_override` 重新注入，agent 无需"记住"模式 |
...
**状态刷新：** `useAppStatus` 每 **30s** 轮询 `get_app_status`；并在 `turn-complete`、`session-resumed`、`permission-mode-changed`、**任意工具 `tool-call-request` phase=`result`** 时即时 **全量 refresh**（后者使 TodoWrite 完成后待办立即出现在 StatusBar）。`session-tokens-updated` 仅**局部更新** token 四字段，不触发完整 `get_app_status`。
...
**前端 invoke 参数：** camelCase（例：`resume_session({ sessionId })`、`get_session_transcript({ sessionId: null })`、`update_session_todo({ todoId, status })`、`answer_question({ toolCallId, answers: { selections, customText } })`）。
...
| `turn-complete` | `useAgent` + `useAppStatus` | 含 token 统计；`phase: start` / `error` 分支；`useAgent` 在 pending 工具/问答时跳过 hydrate |
| `session-resumed` | `useAgent` + `useAppStatus` | `useAgent` 清 streaming 并 `hydrateMessages` |
...
| `compaction-progress` | `useCompactionProgress` | ChatPanel `CompactionBanner` |

**聊天区 UX（纯前端）：** `ScrollViewport` 贴底滚动；最后一轮 `transcript-turn-anchor` 占满视口高度以折叠较早气泡；用户气泡滚出视口上方时显示 **sticky-prompt-header**（点击平滑滚回本轮提问）。`AskUserQuestion` 全宽卡片插在 `pauseAfterSegmentId` 对应段 tools 之后。

**`session-resumed` 事件：** `create_session` / `resume_session` / 切换作品成功后触发；`useAgent` 清 streaming 状态并 `hydrateMessages`（`get_session_transcript` → archive + active）。`App.tsx` 在 `sessionId` 变化时也会 hydrate（略冗余但无害）。

**未使用 IPC：** `get_session_messages` 已注册，UI 统一走 `get_session_transcript`。
