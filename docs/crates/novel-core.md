...
`AgentEngine` 管理完整的小说创作会话。创建时加载作品配置与 Skill、构建 24 个工具的 ToolRegistry、`assemble_system_prompt` 拼装 system（静态 + 动态段），**立即** persist `(0,0)` system 与 `system_static_frozen` metadata。`resume`：`stored` 非空时从 SQLite 加载**冻结** system（`resume_load_stored_messages`，禁止 `build_initial_prompt` 覆盖）；`stored` 为空时走 `resume_bootstrap_empty_messages`（与 `new` 相同的一次性 bootstrap）。`turn_number` 从已有消息推导；**`read_cache/sync`** 在 resume 时 hydrate 子表或 rebuild 保留 ReAct 切片内的 Read/Tail/Write/Edit cache。
...
| `context_tokens` | 主 Agent 本次 API 的 `hit+miss+comp` 快照（SubAgent 与压缩摘要 LLM 计费不覆盖） |

内存侧 `last_context_tokens` 供 compaction 阈值判断。每次 LLM 调用经 `apply_session_usage` 累计后 **始终** emit `session-tokens-updated`（主 Agent 与 SubAgent）；SubAgent 与压缩摘要 API 仅累加计费三项，不覆盖 `context_tokens`。压缩结束后 `last_context_tokens` 置 0 防重入；**下一轮主 inner_turn API**（`record_usage(..., true)`）再写入压缩后工作集的 `context_tokens` 并推送。StatusBar token 以该事件为主路径；`get_app_status` **30s** 轮询与 `turn-complete` / `permission-mode-changed` refresh 兜底 turn 标志等。**Todos** 由 `session-todos-updated` 驱动（`session_todos.rs` + `poll_ui_results` / `apply_ok_stream_result`）；`TodoWrite` 后即时推送，不经 turn 结束 `get_app_status`。`replace=true` 新建一批；`replace=false` 仅更新已有 id（未知 id → tool 结果 `warning`）。Turn 末 `maybe_inject_todo_nudge`（最多 3 次 hidden user）督促清零未完成项。
...
**`EngineShared` 子 Agent 相关字段：** `subagent_queue`、`session_llm`、`drain_in_progress`、`fork_stream_subs`（与 `AppState` 共享 `Arc`）；`EngineStatus.hook_running` 与之同义（历史字段名，覆盖全部 subagent drain）。详见 [FRAMEWORK.md](../../FRAMEWORK.md)。
...
| Agent | max_react_loops | 建议优先工具 |
|-------|-----------------|--------------|
| **PlanAuditor** | 30 | Read/Grep/PlotGraph/ForeshadowTracker/CharacterSearch/TrackingQuery/RelationQuery/Corkboard/Tail/Stats（只读） |
| KnowledgeAuditor | 40（settings `knowledge_auditor_max_react_loops` 可覆盖） | Read/Grep/CharacterSearch/PlotGraph/Tail/TrackingQuery/RelationQuery/ForeshadowTracker（只读） |
...
**动态上下文：** agents_md、knowledge_index（≤2000 字符）、memory（≤4KB，由 [`novel-memory::load_memory`](novel-memory.md) 加载，截断时追加 WARNING）、progress（章节进度 + **未完成**会话待办 `pending`/`in_progress`）、skill_summaries（仅 name+description 摘要，不含 body；**压缩时读盘刷新**）。
...
`AskUserQuestionTool` 返回 `NeedsUserInput` 时，前端展示问答面板，turn 暂停。用户回答后 `answer_question` 写入 tool result，`continue_turn_loop` 续跑（inner turn 预算按当前 turn 内已消耗量计算）。`pending_user_question` 非空时不 emit `TurnComplete`。`maybe_spawn_memory_extraction` 在 turn 暂停时不 spawn；`answer_question` 后续跑完且 turn 真正结束后触发。提取任务正文见 `prompt/memory/extraction-task.md`（含 AskUserQuestion 优先条款）；`had_ask_user_question` 绕开「主 Agent 已写 memory」跳过与节流门控。
...
src/
  ...
  agent/            catalog, prompt (fork task_message), AgentType
  subagent/         fork_setup, queue/drain, runner (ReAct)
  context/          manager, system_prompt, dynamic_context/
  engine/           types, lifecycle, run_loop, prompt_permission, session, session_llm
  turn/             context, loop/ (compaction, compaction_helpers, inner_turn), llm_stream, tool_dispatch, tool_apply, tool_merge
...
| SessionTokensUpdated | LLM 计费后推送（→ `session-tokens-updated`） |
| SessionTodosUpdated | TodoWrite / `update_session_todo` 后推送全量 todos（→ `session-todos-updated`） |
...
| SubAgentStreamDelta / SubAgentToolUpdate | 子 Agent 流式正文与工具；经 `ForkStreamSubscriptions` gate，未订阅不 emit |
| InterruptibleStatusChanged | 主会话可中断工具标志变更（低频，不合并） |
| CompactionProgress | 压缩进度（`compaction-progress` → CompactionBanner；payload：`started` / `generating-summary` / `rebuilding-session` / `done`（含 `epoch`、`retainedMinTurn`/`retainedMaxTurn`） / `failed`） |
