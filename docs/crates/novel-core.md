...
**审计台账：** ToolFork 类 Subagent 注入报告时，`subagent/mod.rs` 调用 `novel_knowledge::mark_audited` 写入 `knowledge/meta/audit-status.md`（`已审计`）；主 Agent 修复后 prompt 要求标 `已通过` + append 修复记录。`build_dynamic_context` Progress 段含 `format_progress_hint` 摘要。
