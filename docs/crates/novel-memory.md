...
1. 主 Agent 本轮已写 `memory/` → 跳过，**除非**本轮含 `AskUserQuestion`（用户决策须复核）
2. 节流门控（`pass_throttle_gate`）—— `EXTRACTION_THROTTLE_TURNS` 控制频率；**含 AskUserQuestion 时绕过节流**
3. 已有提取进行中 → 归并待处理 context（`coalesce_pending`）
4. 无新消息 → 跳过
5. CAS 获取进行中锁 → 构建 prompt（`build_memory_extraction_prompt` + `extraction-task.md`）

`novel-core::maybe_spawn_memory_extraction` 在 turn **未**因 AskUserQuestion / 待审批而暂停时才 spawn；`answer_question` → `continue_turn_loop` 正常结束后同样触发，确保用户作答已进入 transcript。
